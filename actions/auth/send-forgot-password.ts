...
import {sendResetPasswordToken, generateResetPasswordToken} from "@/lib/reset-password-token";
...
export const sendForgotPassword = async (data: ForgotPasswordSchemaType) => {
    ...
    const {error} = await sendResetPasswordToken(passwordResetToken.email, passwordResetToken.token);
    if (error) {
